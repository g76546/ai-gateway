...
async function recordModelFailure(env: Env, providerId: string, modelId: string, status: number, errorMsg: string) {
  ...
  // 如果当前模型在第一梯队中且出现异常，实时踢出第一梯队并触发第二梯队自动补位
  const fullId = `${providerId}/${modelId}`
  let storage = await getTierStorage(env)
  if (storage && storage.tier1.some((m) => m.fullId === fullId)) {
    console.log(`[proxy] 第一梯队模型 ${fullId} 调用异常(${status})，立即实时剔除并补充备用模型`)
...
/** 针对单具体模型 (providerId/modelId) 执行真实 API 代理转发 */
async function executeSingleProxy(
  ...
  if (!parsed) {
    await recordLog(c.env, startTime, requestedModelLabel, 400, `模型格式错误 "${model}"`)
    return c.json({
      error: {
  ...
  if (!provider) {
    await recordLog(c.env, startTime, requestedModelLabel, 404, `提供商 "${providerId}" 不存在`)
    return c.json({
  ...
  if (!provider.enabled) {
    await recordLog(c.env, startTime, requestedModelLabel, 403, `提供商 "${provider.name}" 已禁用`)
    return c.json({
  ...
  if (!modelConfig) {
    await recordLog(c.env, startTime, requestedModelLabel, 404, `模型 "${modelId}" 未配置`)
    return c.json({
      error: { message: `模型 "${modelId}" 未在提供商 "${provider.name}" 中配置`, type: 'invalid_request_error' },
    }, 404)
  }
  if (!modelConfig.enabled) {
    await recordLog(c.env, startTime, requestedModelLabel, 403, `模型 "${modelId}" 已禁用`)
    return c.json({
  ...
  if (modelConfig.permanentlyDisabled) {
    ...
    await recordLog(c.env, startTime, requestedModelLabel, 403, `模型已标记永久失效 (${reason})`)
    return c.json({
  ...
  if (modelConfig.cooldownUntil && Date.now() < modelConfig.cooldownUntil) {
    ...
    await recordLog(c.env, startTime, requestedModelLabel, 530, `模型处于冷却期 (${remainingSec}s)`)
    return c.json({
  ...
  if (isOpenCodeProvider(providerId)) {
    ...
    if (isContentEmpty) {
      ...
      await recordLog(c.env, startTime, requestedModelLabel, 502, errReason)
    ...
    if (!response.ok) {
      await recordModelFailure(c.env, providerId, modelId, response.status, errReason || 'OpenCode 请求失败')
    }
    await recordLog(c.env, startTime, requestedModelLabel, response.status, errReason)
  ...
  if (enabledKeys.length === 0) {
    await recordLog(c.env, startTime, requestedModelLabel, 500, `提供商 "${provider.name}" 未配置可用的 API Key`)
    return c.json({
  ...
  for (const keyIndex of keyOrder) {
    ...
    try {
      ...
      // 5xx 服务器/上游崩溃错误：模型层面严重故障，记录并直接跳出
      ...
      await recordLog(c.env, startTime, requestedModelLabel, response.status, errMsg)
      await recordBusinessLatency(c.env, `${providerId}/${modelId}`, Date.now() - startTime, false, isAutoRequest, sessionId)
  ...
  if (lastError) {
    ...
    await recordLog(c.env, startTime, requestedModelLabel, lastError.status || 502, errMsg)
    await recordBusinessLatency(c.env, `${providerId}/${modelId}`, Date.now() - startTime, false, isAutoRequest, sessionId)
    return c.json({
      error: {
        message: errMsg,
  ...
  await recordLog(c.env, startTime, requestedModelLabel, 500, '没有可用的 API Key')
  return c.json({
...
export async function handleProxy(c: Context<{ Bindings: Env }>) {
  ...
  try {
    ...
    if (!rawModel) {
      await recordLog(c.env, startTime, 'unknown', 400, '缺少 model 参数')
      return c.json({ error: { message: '缺少 model 参数', type: 'invalid_request_error' } }, 400)
    }

    if (rawModel === 'auto' || rawModel === 'auto/auto') {
      isAutoRequest = true
      const triedFullIds = new Set<string>()
      const maxAttempts = 3
      let lastAutoErrorRes: Response | null = null

      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const autoRes = await selectAutoModel(c.env, isLongText, sessionId, triedFullIds)
        if (!autoRes) {
          if (lastAutoErrorRes) return lastAutoErrorRes
          await recordLog(c.env, startTime, 'auto', 503, '第一梯队无可用的模型')
          return c.json({ error: { message: '第一梯队池暂无可用的模型，请先配置模型或进行初始化探测', type: 'service_unavailable' } }, 503)
        }

        triedFullIds.add(autoRes.fullId)
        const currentRequestedModel = `auto (${autoRes.fullId})`

        const res = await executeSingleProxy(c, body, autoRes.fullId, startTime, currentRequestedModel, isAutoRequest, sessionId)
        if (res.ok) {
          return res
        }

        lastAutoErrorRes = res
        console.warn(`[proxy] Auto mode candidate ${autoRes.fullId} failed with status ${res.status}. Retrying next auto candidate...`)
      }

      return lastAutoErrorRes || c.json({ error: { message: '第一梯队所有备选模型尝试均失败', type: 'auto_routing_failed' } }, 502)
    } else {
      return await executeSingleProxy(c, body, rawModel, startTime, rawModel, isAutoRequest, sessionId)
    }
  } catch (err) {
    const error = err as Error
    await recordLog(c.env, startTime, 'unknown', 500, error.message || '代理转发内部错误')
    return c.json({
