...
3. **双梯队智能自动路由 (`auto/auto`)**：首创第一梯队（Tier 1，上限 9 席）与第二梯队（Tier 2，备用池）双层模型池架构，结合真实业务延迟与智能调度算法，实现高可用与低延迟转发。
...
* **动态淘汰规则**：
  * **失败淘汰规则**：当第一梯队模型发生 1 次业务请求失败时，立即将其移出第一梯队，标黄并进入 10 分钟冷却倒计时。
  * **自动海选补位**：触发淘汰后，自动启动海选机制从第二梯队中补充优质可用模型填补空位（上限 9 席）。
